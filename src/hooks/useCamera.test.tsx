...
function renderCamera(options: CameraProviderOptions = {}) {
  return renderHook(() => useCamera(), {
    wrapper: ({ children }) => <CameraProvider {...options}>{children}</CameraProvider>,
  });
}
...
describe("useCamera", () => {
  ...
  describe("panBy", () => {
    ...
    it("adds to an existing pan offset", () => {
      const { result } = renderCamera({ initialPan: { x: 100, y: 200 } });
  ...
  describe("setZoom", () => {
    ...
    it("clamps zoom below minZoom to minZoom", () => {
      const { result } = renderCamera({ minZoom: 0.5 });
    ...
    it("clamps zoom above maxZoom to maxZoom", () => {
      const { result } = renderCamera({ maxZoom: 4 });
  ...
  describe("zoomAt", () => {
    ...
    it("does not change pan when zoom level is already at the target", () => {
      const { result } = renderCamera({ initialZoom: 2, initialPan: { x: 50, y: 50 } });
